...
public class TestMovieAndMovieDataReader {
   ...
   * contains all 3 Description of the source data in any order. It fails
   * if the list returned is missing one or more Description or if an
   * exception occurs while reading in the data.
   * @return true if the test passed, false if it failed
   */
    public boolean testGenre() {
      List<MovieInterface> movieList;
      try {
        movieList = readerToTest.readDataSet(new StringReader(
            "title,original_title,year,genre,duration,country,language,director,writer,production_company,actors,description,avg_vote\n"
      ...
      String Genres1 = "Horror";
      String Genres2 = "Action";
      String Genres3 = "Comedy, Musical, Romance";
      boolean equalOne = true;
      // check if first movie is has of the above Genres
      equalOne = equalOne
          && (Genres1.equals(movieList.get(0).getGenres()) || Genres2.equals(movieList.get(0).getGenres())
              || Genres3.equals(movieList.get(0).getGenres()));
      // check if second movie is has of the above Genres
      equalOne = equalOne
          && (Genres1.equals(movieList.get(1).getGenres()) || Genres2.equals(movieList.get(1).getGenres())
              ||Genres3.equals(movieList.get(1).getGenres()));
      // check if third movie is has of the above Genres
      equalOne = equalOne
          && (Genres1.equals(movieList.get(2).getGenres()) || Genres2.equals(movieList.get(2).getGenres())
    ...
    /**
     * This test reads in 3 movies and tests whether the list of movies
     * contains all 3 director of the source data in any order. It fails
